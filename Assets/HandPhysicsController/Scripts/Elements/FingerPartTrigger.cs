...
namespace HandPhysicsExtenstions
{
    ...
    public class FingerPartTrigger : MonoBehaviour
    {
        ...
        public GameObject NextSumbol;

        public bool _IsFingerPads;
        public HandPhysicsController handPhysicsController;

        void OnTriggerEnter(Collider col)
        {
            if (col.gameObject.tag == "ButtonsSchulte")        
                col.gameObject.GetComponent<Button>().onClick.Invoke();

            if (col.gameObject.tag == _tag && _IsFingerPads && handPhysicsController)
            {
                handPhysicsController.StartBendFinger(FingerType.Thumb);
           //   handPhysicsController.StartBendFinger(FingerType.Index);
                handPhysicsController.StartBendFinger(FingerType.Middle);
                handPhysicsController.StartBendFinger(FingerType.Ring);
                handPhysicsController.StartBendFinger(FingerType.Pinky);
            }
        ...
        void OnTriggerExit(Collider col)
        {
            if (col.gameObject.tag == _tag && _IsFingerPads && handPhysicsController)
            {
                handPhysicsController.StopBendFinger(FingerType.Thumb);
                handPhysicsController.StopBendFinger(FingerType.Index);
                handPhysicsController.StopBendFinger(FingerType.Middle);
                handPhysicsController.StopBendFinger(FingerType.Ring);
                handPhysicsController.StopBendFinger(FingerType.Pinky);
            }
